...
public class PerformanceBenchmarkService {
    ...
    public Path generateReportFromResults(List<BenchmarkResult> results, ReportFormat format) throws Exception {
        return switch (format) {
            case MARKDOWN -> writeTemp(buildReportMarkdown(results), "performance_benchmark_", ".md");
            case CSV      -> writeTemp(buildReportCsv(results),      "performance_benchmark_", ".csv");
            case PDF      -> buildReportPdf(results);
        };
    }
    ...
    private Path buildReportPdf(List<BenchmarkResult> results) throws Exception {
        String ts = LocalDateTime.now().format(TIMESTAMP_FMT);
